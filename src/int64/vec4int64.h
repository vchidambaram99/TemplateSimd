...
namespace tsimd{
    ...
    public:
        ...
        TSIMD_INLINE intl::ConstProxy<int64_t,4> operator[](const std::size_t idx) const { return intl::ConstProxy<int64_t,4>(*this,idx); }
        TSIMD_INLINE intl::AssignmentProxy<int64_t,4> operator[](const std::size_t idx){ return intl::AssignmentProxy<int64_t,4>(*this,idx); }
        TSIMD_INLINE static vec<int64_t,4> zero(){ return _mm256_setzero_si256(); }
        ...
        TSIMD_INLINE masktype operator==(const vec<int64_t,4> a) const {
        ...
        TSIMD_INLINE vec<int64_t,4> max(const vec<int64_t,4> rhs) const {
            return select(*this>rhs,*this,rhs);
        }
        TSIMD_INLINE vec<int64_t,4> min(const vec<int64_t,4> rhs) const {
            return select(*this>rhs,*this,rhs);
        }
        __m256i data;
        enum{ size = 4 };
