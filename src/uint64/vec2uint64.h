...
namespace TSimd{
    ...
    public:
        ...
        TSIMD_INLINE vec(uint64_t* a){ data = _mm_loadu_si128((__m128i*)a); }
        TSIMD_INLINE vec(__m128i a){ data = a; }
        TSIMD_INLINE vec(uint64_t a, uint64_t b){ data = _mm_set_epi64x(b,a); }
        TSIMD_INLINE void store(uint64_t* a){ _mm_storeu_si128((__m128i*)a,data); }
        ...
        inline vec<uint64_t,2>& operator/=(const vec<uint64_t,2>& rhs){ //TODO find simd integer division algorithm
            (*this)[0]/=rhs[0];
            (*this)[1]/=rhs[1];
            return *this;
        }
        ...
        TSIMD_INLINE vec<uint64_t,2> operator/(const vec<uint64_t,2>& rhs) const { //TODO find simd integer division algorithm
