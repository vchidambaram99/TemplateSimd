#ifndef TSIMD_VEC16S_H
#define TSIMD_VEC16S_H
...
namespace TSimd{
    ...
    public:
        ...
        TSIMD_INLINE vec<short,16>& operator<<=(const int& shift){
        ...
        TSIMD_INLINE vec<short,16> operator<<(const int& shift) const {
            return vec<short,16>(_mm256_slli_epi16(data,shift));
        }
        TSIMD_INLINE vec<short,16> operator>>(const int& shift) const {
