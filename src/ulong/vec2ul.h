#ifndef TSIMD_VEC2UL_H
#define TSIMD_VEC2UL_H
...
namespace TSimd{
    ...
    public:
        ...
        TSIMD_INLINE vec<unsigned long,2>& operator<<=(const int& shift){
        ...
        TSIMD_INLINE vec<unsigned long,2> operator<<(const int& shift) const {
            return vec<unsigned long,2>(_mm_slli_epi64(data,shift));
        }
        TSIMD_INLINE vec<unsigned long,2> operator>>(const int& shift) const {
